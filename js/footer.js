
import { geturl } from "./functions.js";
let thebody = document.querySelector('div#body')
let thef = document.createElement('div');
thef.className = 'the-footer bc-white  bsbb w-100 h-a bc-white';
thebody.appendChild(thef);
thef.innerHTML = `<div class="bc-white w-90 m-a bfull-resp h-100 black pt-20p bsbb">
        <div class=" bsbb w-400p bfull-resp p-10p igrid">
            <div class="title w-100 h-a  p-r">
                <div class="w-40p h-40p p-5p bsbb p-r ovh br-50 igrid">
                    <font class="center w-100 h-100">
                        <svg class="w-30p h-30p rotate-320" viewBox="0 0 64 64" xmlns="http://www.w3.org/2000/svg" stroke="#ccc"><g id="a"/><g id="b"><path d="M50,16.0146v-5.5253c0-.1943-.1123-.3706-.2881-.4531-.1748-.0811-.3848-.0552-.5322,.0693-6.8387,5.7183-17.4261,6.855-17.7166,6.885l-11.8411,.7893c-1.5962,.1064-3.1899,.3672-4.7363,.7749-1.1102,.2924-1.8854,1.2913-1.8857,2.4287h-.5001c-2.4814,0-4.5,2.3467-4.5,5.2314v3.5483c0,2.8848,2.0186,5.2314,4.5,5.2314h.5001c.0002,1.1374,.7755,2.1363,1.8857,2.4287,.9074,.2393,1.832,.4214,2.763,.558,.5023,.4549,2.34,2.2903,3.0822,5.4103,.6973,2.9302,.1816,5.8906-1.4517,8.3369-.2988,.4478-.3257,.999-.0713,1.4741,.2603,.4868,.7695,.7891,1.3291,.7891h8.9331c.4438,0,.8623-.1899,1.1479-.5215,.2764-.3208,.3994-.7437,.3374-1.1606-.3254-2.1784-1.2852-8.5849-3.4286-13.584l3.9362,.2625c.2903,.03,10.8778,1.1662,17.7166,6.885,.0908,.0767,.2051,.1162,.3203,.1162,.0723,0,.1445-.0156,.2119-.0469,.1758-.0825,.2881-.2588,.2881-.4531v-5.5253c3.3505-.257,6-3.0594,6-6.4747v-11c0-3.4152-2.6495-6.2177-6-6.4747ZM9,29.7632v-3.5483c0-2.333,1.5703-4.2314,3.5-4.2314h.5v12.0112h-.5c-1.9297,0-3.5-1.8984-3.5-4.2314Zm20.9663,22.6938c.0195,.1309-.0181,.2588-.106,.3604-.0952,.1108-.2378,.1743-.3901,.1743h-8.9331c-.1929,0-.3604-.0977-.4473-.2607-.0371-.0693-.1094-.2515,.0215-.4468,1.7891-2.6802,2.355-5.9204,1.5923-9.124-.5588-2.3491-1.6739-4.0107-2.5112-5.0009,.1433,.012,.2863,.0294,.4296,.039l6.7756,.4519c1.4865,3.2765,2.6885,7.9119,3.5687,13.8069Zm1.0337-14.5024l-11.3115-.7544c-1.5327-.1021-3.063-.3521-4.5479-.7437-.6714-.1772-1.1406-.7783-1.1406-1.4624v-14.0103c0-.6841,.4692-1.2852,1.1406-1.4624,1.4849-.3916,3.0151-.6416,4.5479-.7437l11.3115-.7539v19.9307Zm18,6.4971c-6.3689-4.8357-14.9624-6.1545-17-6.4078V17.9346c2.0376-.2532,10.6311-1.5725,17-6.4078V44.4517Zm6-10.9624c0,2.8639-2.2014,5.2208-5,5.4747V17.0146c2.7986,.2538,5,2.6108,5,5.4747v11Z"/></g></svg>
                    </font>
                </div>
                <div class="igrid h-40p t-0 p-a center ml-60p">
                    <font class="fs-26p bold capitalize verdana ">
                        about us !
                    </font>
                </div>
            </div>
            <div class="w-100 h-a bsbb p-10p">
                <div class="flex h-100 w-100">
                    <div class="w-a p-r h-100  p-20p bsbb">
                        <img class="w-80p h-80p contain" src="${geturl()}/icons/favicon.png">
                    </div>
                    <span class="fs-12p verdana dgray h-100">IT SPACE LTD is a Rwandan technology company which deals with ,  SUPPLY-INSTALLATION-MAINTAINACE of all ICT & electronic devices, ICT consultancy services, as well as logistics services, Web services Training and internships.<br>
                    our main goal is to serve our current customers and at all potential customers on socioeconomic levels sustainably. We believe that technology is a 7th sense that human use to create and recreate all possible solutions that the world is facing in nowa days. 
                    </span>
                </div>
            </div>
        </div>
        <div class=" bsbb w-400p bfull-resp p-10p igrid">
            <div class="title w-100 h-a  p-r">
                <div class=" w-40p h-40p p-5p bsbb p-r ovh br-50 igrid">
                    <font class="center w-100 h-100">
                        <svg class="w-30p h-30p rotate-320" viewBox="0 0 64 64" xmlns="http://www.w3.org/2000/svg" stroke="#ccc"><g id="a"/><g id="b"><path d="M50,16.0146v-5.5253c0-.1943-.1123-.3706-.2881-.4531-.1748-.0811-.3848-.0552-.5322,.0693-6.8387,5.7183-17.4261,6.855-17.7166,6.885l-11.8411,.7893c-1.5962,.1064-3.1899,.3672-4.7363,.7749-1.1102,.2924-1.8854,1.2913-1.8857,2.4287h-.5001c-2.4814,0-4.5,2.3467-4.5,5.2314v3.5483c0,2.8848,2.0186,5.2314,4.5,5.2314h.5001c.0002,1.1374,.7755,2.1363,1.8857,2.4287,.9074,.2393,1.832,.4214,2.763,.558,.5023,.4549,2.34,2.2903,3.0822,5.4103,.6973,2.9302,.1816,5.8906-1.4517,8.3369-.2988,.4478-.3257,.999-.0713,1.4741,.2603,.4868,.7695,.7891,1.3291,.7891h8.9331c.4438,0,.8623-.1899,1.1479-.5215,.2764-.3208,.3994-.7437,.3374-1.1606-.3254-2.1784-1.2852-8.5849-3.4286-13.584l3.9362,.2625c.2903,.03,10.8778,1.1662,17.7166,6.885,.0908,.0767,.2051,.1162,.3203,.1162,.0723,0,.1445-.0156,.2119-.0469,.1758-.0825,.2881-.2588,.2881-.4531v-5.5253c3.3505-.257,6-3.0594,6-6.4747v-11c0-3.4152-2.6495-6.2177-6-6.4747ZM9,29.7632v-3.5483c0-2.333,1.5703-4.2314,3.5-4.2314h.5v12.0112h-.5c-1.9297,0-3.5-1.8984-3.5-4.2314Zm20.9663,22.6938c.0195,.1309-.0181,.2588-.106,.3604-.0952,.1108-.2378,.1743-.3901,.1743h-8.9331c-.1929,0-.3604-.0977-.4473-.2607-.0371-.0693-.1094-.2515,.0215-.4468,1.7891-2.6802,2.355-5.9204,1.5923-9.124-.5588-2.3491-1.6739-4.0107-2.5112-5.0009,.1433,.012,.2863,.0294,.4296,.039l6.7756,.4519c1.4865,3.2765,2.6885,7.9119,3.5687,13.8069Zm1.0337-14.5024l-11.3115-.7544c-1.5327-.1021-3.063-.3521-4.5479-.7437-.6714-.1772-1.1406-.7783-1.1406-1.4624v-14.0103c0-.6841,.4692-1.2852,1.1406-1.4624,1.4849-.3916,3.0151-.6416,4.5479-.7437l11.3115-.7539v19.9307Zm18,6.4971c-6.3689-4.8357-14.9624-6.1545-17-6.4078V17.9346c2.0376-.2532,10.6311-1.5725,17-6.4078V44.4517Zm6-10.9624c0,2.8639-2.2014,5.2208-5,5.4747V17.0146c2.7986,.2538,5,2.6108,5,5.4747v11Z"/></g></svg>
                    </font>
                </div>
                <div class="igrid h-40p t-0 p-a center ml-60p">
                    <font class="fs-26p bold capitalize verdana">
                        stay connected !
                    </font>
                </div>
            </div>
            <div class="w-100 h-70p">
                <ul class="ls-none flex pl-20p">
                    <li class="pt-0 pl-5p pr-5p">
                        <span class="icon #facebook-icon w-30p h-30p center">
                            <svg xmlns="http://www.w3.org/2000/svg" fill="#555555" class="hover-5" width="20" height="20" viewBox="0 0 24 24">
                            <path d="M12 2.03998C6.5 2.03998 2 6.52998 2 12.06C2 17.06 5.66 21.21 10.44 21.96V14.96H7.9V12.06H10.44V9.84998C10.44 7.33998 11.93 5.95998 14.22 5.95998C15.31 5.95998 16.45 6.14998 16.45 6.14998V8.61998H15.19C13.95 8.61998 13.56 9.38998 13.56 10.18V12.06H16.34L15.89 14.96H13.56V21.96C15.9164 21.5878 18.0622 20.3855 19.6099 18.57C21.1576 16.7546 22.0054 14.4456 22 12.06C22 6.52998 17.5 2.03998 12 2.03998Z"/>
                            </svg>
                        </span>
                    </li>
                    <li class="pt-0 pl-5p pr-5p">
                        <span class="icon #twitter-icon w-30p h-30p center">
                            <svg xmlns="http://www.w3.org/2000/svg" class="hover-5" fill="#555555" width="20" height="20" xmlns:xlink="http://www.w3.org/1999/xlink" version="1.1" id="Layer_1" x="0px" y="0px" viewBox="0 0 310 310" xml:space="preserve">
                                <path d="M302.973,57.388c-4.87,2.16-9.877,3.983-14.993,5.463c6.057-6.85,10.675-14.91,13.494-23.73   c0.632-1.977-0.023-4.141-1.648-5.434c-1.623-1.294-3.878-1.449-5.665-0.39c-10.865,6.444-22.587,11.075-34.878,13.783   c-12.381-12.098-29.197-18.983-46.581-18.983c-36.695,0-66.549,29.853-66.549,66.547c0,2.89,0.183,5.764,0.545,8.598   C101.163,99.244,58.83,76.863,29.76,41.204c-1.036-1.271-2.632-1.956-4.266-1.825c-1.635,0.128-3.104,1.05-3.93,2.467   c-5.896,10.117-9.013,21.688-9.013,33.461c0,16.035,5.725,31.249,15.838,43.137c-3.075-1.065-6.059-2.396-8.907-3.977   c-1.529-0.851-3.395-0.838-4.914,0.033c-1.52,0.871-2.473,2.473-2.513,4.224c-0.007,0.295-0.007,0.59-0.007,0.889   c0,23.935,12.882,45.484,32.577,57.229c-1.692-0.169-3.383-0.414-5.063-0.735c-1.732-0.331-3.513,0.276-4.681,1.597   c-1.17,1.32-1.557,3.16-1.018,4.84c7.29,22.76,26.059,39.501,48.749,44.605c-18.819,11.787-40.34,17.961-62.932,17.961   c-4.714,0-9.455-0.277-14.095-0.826c-2.305-0.274-4.509,1.087-5.294,3.279c-0.785,2.193,0.047,4.638,2.008,5.895   c29.023,18.609,62.582,28.445,97.047,28.445c67.754,0,110.139-31.95,133.764-58.753c29.46-33.421,46.356-77.658,46.356-121.367   c0-1.826-0.028-3.67-0.084-5.508c11.623-8.757,21.63-19.355,29.773-31.536c1.237-1.85,1.103-4.295-0.33-5.998   C307.394,57.037,305.009,56.486,302.973,57.388z"/>
                            </svg>
                        </span>
                    </li>
                    <li class="pt-0 pl-5p pr-5p">
                       <span class="icon #instagram-icon w-30p h-30p center">
                            <a href="https://www.instagram.com/itspace_rwanda/">
                                    <svg xmlns="http://www.w3.org/2000/svg"class="hover-5" width="20" height="20" viewBox="0 0 24 24">
                                        <path d="M16.19 2H7.81C4.17 2 2 4.17 2 7.81V16.18C2 19.83 4.17 22 7.81 22H16.18C19.82 22 21.99 19.83 21.99 16.19V7.81C22 4.17 19.83 2 16.19 2ZM12 15.88C9.86 15.88 8.12 14.14 8.12 12C8.12 9.86 9.86 8.12 12 8.12C14.14 8.12 15.88 9.86 15.88 12C15.88 14.14 14.14 15.88 12 15.88ZM17.92 6.88C17.87 7 17.8 7.11 17.71 7.21C17.61 7.3 17.5 7.37 17.38 7.42C17.26 7.47 17.13 7.5 17 7.5C16.73 7.5 16.48 7.4 16.29 7.21C16.2 7.11 16.13 7 16.08 6.88C16.03 6.76 16 6.63 16 6.5C16 6.37 16.03 6.24 16.08 6.12C16.13 5.99 16.2 5.89 16.29 5.79C16.52 5.56 16.87 5.45 17.19 5.52C17.26 5.53 17.32 5.55 17.38 5.58C17.44 5.6 17.5 5.63 17.56 5.67C17.61 5.7 17.66 5.75 17.71 5.79C17.8 5.89 17.87 5.99 17.92 6.12C17.97 6.24 18 6.37 18 6.5C18 6.63 17.97 6.76 17.92 6.88Z" fill="#555555"/>
                                    </svg>
                            </a>
                       </span>
                    </li>
                    <li class="pt-0 pl-5p pr-5p">
                        <span class="icon #youtube-icon w-30p h-30p iblock">
                            <svg xmlns="http://www.w3.org/2000/svg" fill="#555555" class="hover-5" width="30" height="30" viewBox="0 0 32 32"><path d="M24.325 8.309s-2.655-.334-8.357-.334c-5.517 0-8.294.334-8.294.334A2.675 2.675 0 0 0 5 10.984v10.034a2.675 2.675 0 0 0 2.674 2.676s2.582.332 8.294.332c5.709 0 8.357-.332 8.357-.332A2.673 2.673 0 0 0 27 21.018V10.982a2.673 2.673 0 0 0-2.675-2.673zM13.061 19.975V12.03L20.195 16l-7.134 3.975z"/></svg>
                            </span>
                    </li>
                </ul>
            </div>
            <div class="w-100 h-a pl-20p pr-5p bsbb">
                <span class=" fs-12p bsbb verdana">
                Stay up-to-date with our latest news and announcements by following us on social media. We regularly post updates and blogs on our platforms, so be sure to check us out! Connect with us on Facebook, Twitter, and Instagram for instant updates and behind-the-scenes looks at our company. Join our community and never miss a beat.
                </span>
            </div>
        </div>
        <div class=" bsbb w-400p bfull-resp p-10p igrid">
            <div class="title w-100 h-a  p-r">
                <div class="w-40p h-40p p-5p bsbb p-r ovh br-50 igrid">
                    <font class="center w-100 h-100">
                        <svg class="w-30p h-30p" viewBox="0 0 24 24" role="img" xmlns="http://www.w3.org/2000/svg" aria-labelledby="historyIconTitle" stroke="#ccc" stroke-width="1" stroke-linecap="square" stroke-linejoin="miter" fill="none" color="#000000"> <title id="historyIconTitle">History</title> <polyline points="1 12 3 14 5 12"/> <polyline points="12 7 12 12 15 15"/> <path d="M12,21 C16.9705627,21 21,16.9705627 21,12 C21,7.02943725 16.9705627,3 12,3 C7.02943725,3 3,7.02943725 3,12 C3,11.975305 3,12.3086383 3,13"/> </svg>
                    </font>
                </div>
                <div class="igrid h-40p t-0 p-a center ml-60p">
                    <font class="fs-26p bold capitalize verdana">
                        our schedule
                    </font>
                </div>
            </div>
            <div class="w-100 h-a">
                <ul class="ls-none flex pl-15p bsbb w-100">
                    <li class="pt-0 pl-5p pr-5p w-100">
                        <font class="left l-0 verdana capitalize">monday-saturday</font>
                        <font class="right r-0 verdana">8:00 AM - 8:00 PM</font>
                    </li>
                </ul>
            </div>
            <div class="p-20p bsbb">
                <span class="w-100 h-a bsbb verdana fs-12p">
                Our office hours are from 8am to 8pm, Monday through Saturday. If you have any questions or concerns, please feel free to reach on our Headquaters we may help.
                </span>
            </div>
            
        </div>
        <div class=" bsbb w-400p bfull-resp p-10p igrid">
            <div class="title w-100 h-a  p-r">
                <div class="w-40p h-40p p-5p bsbb p-r ovh br-50 igrid">
                    <font class="center w-100 h-100">
                        <svg class="w-30p h-30p" viewBox="0 0 24 24" role="img" xmlns="http://www.w3.org/2000/svg" aria-labelledby="historyIconTitle" stroke="#ccc" stroke-width="1" stroke-linecap="square" stroke-linejoin="miter" fill="none" color="#000000"> <title id="historyIconTitle">History</title> <polyline points="1 12 3 14 5 12"/> <polyline points="12 7 12 12 15 15"/> <path d="M12,21 C16.9705627,21 21,16.9705627 21,12 C21,7.02943725 16.9705627,3 12,3 C7.02943725,3 3,7.02943725 3,12 C3,11.975305 3,12.3086383 3,13"/> </svg>
                    </font>
                </div>
                <div class="igrid h-40p t-0 p-a center ml-60p">
                    <font class="fs-26p bold capitalize verdana">
                        quick links
                    </font>
                </div>
            </div>
            <div class="w-100 h-a">
                <ul class=" ls-none grid pl-20p bsbb w-100">
                    
                    <li class="capitalize w-100 p-10p">
                        <span class="left verdana us-none hover-6 l-0 fs-14p"><a href="${geturl()}/" class="td-none ls-n"><span class="nowrap hover-6 black">home</span></a></span>
                    </li>
                    <li class="capitalize w-100 p-10p">
                        <span class="left verdana us-none hover-6 l-0 fs-14p"><a href="${geturl()}/user/" class="td-none ls-n"><span class="nowrap hover-6 black">my account</span></a></span>
                    </li>
                    <li class="capitalize w-100 p-10p">
                        <span class="left verdana us-none hover-6 l-0 fs-14p"><a href="${geturl()}/about/" class="td-none ls-n"><span class="nowrap hover-6 black">About us</span></a></span>
                    </li>
                    <li class="capitalize w-100 p-10p">
                        <span class="left verdana us-none hover-6 l-0 fs-14p"><a href="${geturl()}/contact/" class="td-none ls-n"><span class="nowrap hover-6 black">contact us</span></a></span>
                    </li>
                    <li class="capitalize w-100 p-10p">
                        <span class="left verdana us-none hover-6 l-0 fs-14p"><a href="${geturl()}/terms/" class="td-none ls-n"><span class="nowrap hover-6 black">terms</span></a></span>
                    </li>
                </ul>
            </div>
        </div>
        <div class=" bsbb w-400p bfull-resp p-10p igrid">
            <div class="title w-100 h-a  p-r">
                <div class="w-40p h-40p p-5p bsbb p-r ovh br-50 igrid">
                    <font class="center w-100 h-100">
                        <svg version="1.1" id="Capa_1" xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink" class="w-30p h-30p" stroke="#ccc" x="0px" y="0px" viewBox="0 0 436.432 436.432" style="fill: #ccc;" xml:space="preserve">
                            <g>
                                <path d="M346.946,74.322C320.077,27.784,271.954,0,218.216,0c-53.737,0-101.86,27.784-128.729,74.322
                                    c-26.869,46.538-26.869,102.106,0,148.644l120.502,208.716c1.697,2.939,4.833,4.75,8.227,4.75s6.53-1.811,8.227-4.75
                                    l120.502-208.716C373.815,176.428,373.815,120.86,346.946,74.322z M330.491,213.466L218.216,407.932L105.941,213.466
                                    c-23.434-40.59-23.434-89.055,0-129.644C129.376,43.232,171.348,19,218.216,19s88.841,24.232,112.275,64.822
                                    C353.926,124.411,353.925,172.876,330.491,213.466z" stroke="#ccc"/>
                                <path d="M218.216,41.193c-59.363,0-107.659,48.296-107.659,107.659c0,59.364,48.296,107.66,107.659,107.66
                                    c59.364,0,107.66-48.296,107.66-107.66C325.876,89.488,277.58,41.193,218.216,41.193z M176.769,227.206v-6.352
                                    c0-22.855,18.594-41.45,41.45-41.45s41.45,18.594,41.45,41.45v6.35c-12.378,6.575-26.486,10.308-41.452,10.308
                                    C203.252,237.511,189.145,233.78,176.769,227.206z M218.216,107.558c14.568,0,26.421,11.853,26.421,26.421
                                    S232.785,160.4,218.216,160.4c-14.569,0-26.421-11.853-26.421-26.421S203.648,107.558,218.216,107.558z M278.27,214.012
                                    c-2.222-19.625-13.875-36.422-30.307-45.752c9.592-8.334,15.674-20.607,15.674-34.281c0-25.045-20.375-45.421-45.421-45.421
                                    s-45.421,20.376-45.421,45.421c0,13.675,6.083,25.949,15.676,34.283c-16.432,9.331-28.085,26.129-30.305,45.754
                                    c-17.579-16.212-28.609-39.422-28.609-65.164c0-48.887,39.772-88.659,88.659-88.659c48.887,0,88.66,39.772,88.66,88.659
                                    C306.876,174.592,295.846,197.801,278.27,214.012z"/>
                            </g>
                        </svg>
                    </font>
                </div>
                <div class="igrid h-40p t-0 p-a center ml-60p">
                    <font class="fs-26p bold capitalize verdana">
                        our address
                    </font>
                </div>
            </div>
            <div class="w-100 h-a pl-20p pr-5p bsbb">
                <ul class="ls-none  p-0 w-100">
                    <li class="pt-5p pl-5p pr-5p w-100 center-2">
                        <div class="p-5p p-r w-20p h-20p iblock">
                            <span class="icon  w-20p h-20p bsbb p-r ovh">
                                <svg version="1.1" id="Capa_1" width="20" height="20" xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink" x="0px" y="0px" viewBox="0 0 55 55" style="fill: var(--main-color);" xml:space="preserve">
                                    <g>
                                        <path d="M20,6.263l15,3.529v40.016l-15-2.647V6.263 M19.001,4.001c-0.224,0-0.443,0.075-0.622,0.216C18.14,4.406,18,4.695,18,5.001
                                            v43c0,0.485,0.348,0.9,0.826,0.984l17,3c0.058,0.011,0.116,0.016,0.174,0.016c0.233,0,0.461-0.082,0.643-0.234
                                            C36.869,51.577,37,51.296,37,51.001v-42c0-0.464-0.319-0.867-0.771-0.974l-17-4C19.153,4.009,19.077,4.001,19.001,4.001
                                            L19.001,4.001z"/>
                                        <g>
                                            <path d="M54.584,2.189c-0.261-0.188-0.597-0.241-0.901-0.137l-18,6C35.275,8.188,35,8.57,35,9.001v42
                                                c0,0.33,0.163,0.639,0.436,0.825c0.168,0.115,0.366,0.175,0.564,0.175c0.123,0,0.245-0.022,0.362-0.068l18-7
                                                C54.747,44.783,55,44.413,55,44.001v-41C55,2.679,54.846,2.378,54.584,2.189z"/>
                                            <path d="M18.783,4.024l-18,4C0.326,8.127,0,8.532,0,9.001v43c0,0.304,0.138,0.59,0.375,0.78c0.179,0.144,0.4,0.22,0.625,0.22
                                                c0.072,0,0.145-0.008,0.217-0.023l18-4C19.674,48.875,20,48.469,20,48.001v-43c0-0.304-0.138-0.59-0.375-0.78
                                                C19.39,4.031,19.08,3.961,18.783,4.024z"/>
                                        </g>
                                    </g>
                                </svg>
                            </span>	
                        </div>
                        <div class="iblock p-5p p-r">
                            <span class="verdana t-0 w-150p fs-12p">Kigali, Rwanda, Nyarugenge, CHIC building floor 0, near access bank, left side shop, IT SPACE, D20-09</span>
                        </div>
                    </li>
                    <li class="w-100 flex">
                        <div class="p-5p p-r w-30p h-a left">
                            <span class="icon  w-100 h-100 center-2 p-r ovh">
                                <svg version="1.1" id="Capa_1" class="w-20p h-20p" xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink" x="0px" y="0px" viewBox="0 0 420.514 420.514" style="stroke: red; fill: var(--main-color);" xml:space="preserve">
                                    <path d="M293.814,254.728l-37.701,28.083l-7.636,24.527c-55.525-33.273-102.028-79.776-135.301-135.301l24.527-7.636l28.083-37.701
                                        L75.978,0L33.567,20.089L0,101.609l0.863,2.632c24.033,73.265,65.555,140.81,120.077,195.332
                                        c54.522,54.522,122.067,96.044,195.332,120.077l2.632,0.863l81.521-33.567l20.088-42.41L293.814,254.728z M147.246,126.481
                                        l-13.948,18.724L50.681,28.58l20.34-9.634L147.246,126.481z M318.327,404.53c-70.028-23.428-134.565-63.348-186.78-115.563
                                        C79.332,236.752,39.412,172.215,15.985,102.186l25.164-61.113l79.908,112.801l-30.221,9.409l4.594,8.095
                                        c36.402,64.154,89.554,117.306,153.708,153.708l8.096,4.594l9.408-30.221l112.801,79.907L318.327,404.53z M391.934,369.833
                                        l-116.626-82.617l18.724-13.948l107.536,76.225L391.934,369.833z"/>
                                </svg>
                            </span>
                        </div>
                        <div class="ml-10p bsbb">
                            <font class="w-100 fs-12p klavika">(+250) 788 247 133</font>
                            <br>
                            <font class="w-100 fs-12p klavika">(+250) 788 543 588</font>
                        </div>
                    </li>
                    <li class="pt-5p pl-5p pr-5p w-100 flex">
                        <div class="p-5p p-r w-30p h-20p igrid">
                            <span class="icon #mail-icon  w-100 h-100 bsbb p-r ovh">
                                <svg version="1.1" id="Capa_1" width="20" height="20" xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink" x="0px" y="0px"  viewBox="0 0 58 58" style="fill: var(--main-color);" xml:space="preserve">
                                    <g>
                                        <path d="M50.466,43.995l6.483-3.242l-17.637-6.498l6.498,17.638l3.242-6.484l6.797,6.797c0.195,0.195,0.451,0.293,0.707,0.293
                                            s0.512-0.098,0.707-0.293c0.391-0.391,0.391-1.023,0-1.414L50.466,43.995z M42.688,37.631l9.232,3.402l-3.887,1.943l-1.943,3.888
                                            L42.688,37.631z"/>
                                        <path d="M57.995,6.477c-0.002-0.111-0.019-0.222-0.06-0.33c-0.003-0.007-0.008-0.012-0.01-0.019
                                            c-0.011-0.027-0.029-0.049-0.042-0.075c-0.041-0.081-0.089-0.155-0.148-0.22c-0.026-0.029-0.054-0.052-0.083-0.078
                                            c-0.062-0.054-0.129-0.099-0.202-0.136c-0.033-0.017-0.063-0.035-0.098-0.048C57.24,5.529,57.124,5.5,57,5.5H1
                                            c-0.124,0-0.24,0.029-0.351,0.071C0.614,5.584,0.584,5.602,0.551,5.619C0.478,5.656,0.411,5.701,0.348,5.755
                                            C0.319,5.781,0.292,5.804,0.266,5.833c-0.06,0.065-0.108,0.14-0.149,0.221C0.104,6.079,0.086,6.101,0.075,6.128
                                            C0.072,6.134,0.067,6.14,0.064,6.146c-0.041,0.108-0.057,0.219-0.06,0.33C0.004,6.485,0,6.492,0,6.5v39
                                            c0,0.003,0.002,0.006,0.002,0.009c0.001,0.107,0.017,0.214,0.053,0.319c0.004,0.012,0.013,0.022,0.018,0.034
                                            c0.013,0.034,0.034,0.063,0.051,0.095c0.039,0.074,0.082,0.142,0.137,0.203c0.028,0.031,0.056,0.058,0.088,0.085
                                            c0.06,0.053,0.126,0.096,0.197,0.133c0.034,0.018,0.066,0.037,0.102,0.051C0.759,46.471,0.876,46.5,1,46.5h35c0.553,0,1-0.447,1-1
                                            s-0.447-1-1-1H3.879c4.548-3.604,14.801-11.781,20.069-16.403l1.421,1.255c1.002,0.894,2.317,1.341,3.633,1.341
                                            c1.313,0,2.626-0.446,3.625-1.337L56,8.718V36.5c0,0.553,0.447,1,1,1s1-0.447,1-1v-30C58,6.492,57.996,6.485,57.995,6.477z
                                            M2,43.437V8.718l20.437,18.046C16.881,31.617,6.086,40.204,2,43.437z M31.299,27.86c-1.267,1.132-3.331,1.132-4.602-0.004
                                            l-1.979-1.747c-0.005-0.007-0.007-0.015-0.012-0.021c-0.022-0.025-0.053-0.035-0.077-0.058L3.644,7.5h50.713L31.299,27.86z"/>
                                    </g>
                                </svg>
                            </span>
                        </div>
                        <div class="iblock p-2p bsbb p-r w-100">
                            <span class="verdana t-0 p-a verdana fs-14p h-100 center"><a href="mailto::itzoneplus250@gmail.com" class="theme td-none ls-n ">itspace250@gmail.com</a></span>
                        </div>
                    </li>
                </ul>
            </div>
            <div class="w-100 h-20p bsbb p-r">
                
            </div>
        </div>
        <div class=" bsbb w-400p bfull-resp p-10p igrid">
            <div class="title w-100 h-a  p-r">
                <div class="w-40p h-40p p-5p bsbb p-r ovh br-50 igrid">
                    <font class="center w-100 h-100">
                        <svg version="1.1" class="w-30p h-30p" style="fill: #ccc;"  xmlns="http://www.w3.org/2000/svg" viewBox="0 0 512 512" xmlns:xlink="http://www.w3.org/1999/xlink" enable-background="new 0 0 512 512">
                        <g>
                            <g>
                            <g>
                                <path d="m473.6,256.6c0-99.3-67.1-185.9-163.3-210.6-9.2-2.4-18.6,3.2-20.9,12.4-2.4,9.2 3.2,18.6 12.4,20.9 80.9,20.7 137.4,93.7 137.4,177.3 0,73.7-43.9,139-109.6,167.7l15.9-37c3.7-8.7-0.3-18.8-9-22.6-8.7-3.7-18.8,0.3-22.6,9l-31,72.2c-3.7,8.7 0.3,18.8 9,22.6l72.2,31c10.8,3.8 19.8-2.5 22.6-9 3.7-8.7-0.3-18.8-9-22.6l-31.4-13.5c76.5-34.6 127.3-111.4 127.3-197.8z"/>
                                <path d="m210.4,433.9c-80.9-20.8-137.4-93.7-137.4-177.3 0-72.7 42.7-137.2 106.9-166.5l-14.9,34.6c-3.7,8.7 0.1,19.2 9,22.6 10.4,3.9 19.2-0.8 22.6-9l31-72.3c3-7 2.9-17.5-9-22.6l-72.2-31c-8.7-3.7-18.8,0.3-22.6,9-3.7,8.7 0.3,18.8 9,22.6l33.6,14.4c-76.8,34.7-127.8,111.6-127.8,198.2 7.10543e-15,99.3 67.2,185.8 163.3,210.6 11.6,3 18.9-4.6 20.9-12.4 2.3-9.2-3.2-18.5-12.4-20.9z"/>
                                <path d="m296.6,321.4c8.8-9.2 13.2-20.8 13.2-34.6 0-6.3-1.1-12.1-3.2-17.5-2.1-5.4-5.1-10-9-14-3.9-3.9-9-7.2-15.2-10-3.3-1.4-9.9-3.4-20-5.9v-57.4c6.5,1.3 11.6,4.1 15.4,8.3 3.7,4.2 6.2,10.2 7.4,18l20.6-3.1c-1.8-12.6-7-22.6-15.7-29.8-6.8-5.7-16-9.1-27.6-10.2v-9.5h-11.8v9.5c-13.2,1.3-23.2,5-29.9,10.9-10.1,8.8-15.1,20.3-15.1,34.5 0,8 1.7,15.1 5.2,21.4 3.4,6.3 8.3,11.2 14.7,14.7 8.6,4.8 17,8 25.2,9.5v63.5c-7.8-0.8-14.6-4.8-20.2-11.9-3.9-5-6.7-12.9-8.2-23.6l-20.1,3.8c0.7,10.3 3.4,19.1 7.9,26.3 4.5,7.2 9.9,12.4 16.2,15.6 6.2,3.2 14.4,5.4 24.3,6.7v19.9h11.8v-20.2c14-0.7 25.4-5.7 34.1-14.9zm-45.8-84.5c-9.1-2.7-15.5-6.4-19.3-11-3.8-4.6-5.7-10.3-5.7-17.1 0-6.9 2.2-12.8 6.7-17.8 4.5-5 10.6-8.1 18.3-9.3v55.2zm30.9,72.6c-5.1,5.8-11.5,9.1-19.2,10.1v-60.7c10.7,3.5 17.9,7.4 21.5,11.8 3.6,4.4 5.4,10.2 5.4,17.5-0.1,8.4-2.6,15.6-7.7,21.3z"/>
                            </g>
                            </g>
                        </g>
                        </svg>

                    </font>
                </div>
                <div class="igrid h-40p t-0 p-a center ml-60p">
                    <font class="fs-26p bold capitalize verdana">
                        accepted payments
                    </font>
                </div>
            </div>
            <div class="w-100 h-a">
                <ul class="ls-none flex p-0 w-100">
                    <li class="pt-0 pl-5p pr-5p w-100">
                        <font class="left l-0 fs-14p verdana">
                            <span class="icon #mtnmomo-icon">
                                <img src="${geturl()}/icons/mtnmomoicon.jpg" class="w-80p h-30p br-5p cover">
                            </span>
                        </font>
                    </li>
                    <li class="pt-0 pl-5p pr-5p w-100">
                        <font class="left l-0 fs-14p verdana">
                            <span class="icon #mtnmomo-icon">
                                <img src="${geturl()}/icons/airtelmoneylogo.png" class="w-80p h-30p contain">
                            </span>
                         </font>
                    </li>
                    <li class="pt-0 pl-5p pr-5p w-100">
                        <font class="left l-0 fs-14p verdana">
                            <span class="icon #mtnmomo-icon">
                                <img src="${geturl()}/icons/visaandmastercard.png" class="w-80p h-30p contain">
                            </span>
                        </font>
                    </li>
                </ul>
            </div>
        </div>
        </div>
        <div class="dev-footer w-100 bsbb">
        <div class="body w-100 h-100 bt-1-s-dgray p-10p bsbb">
            <div class=" bsbb w-100 igrid">
                <div class="w-100 h-a">
                    <p class="ls-none center p-0 w-100 m-0">
                        <span class="left capitalize verdana fs-10p dgray p-10p bsbb center">
                            <span class="center-2">
                                <span class="nowrap hover-6 dgray fs-8p">
                                    &copy; ${new Date().getFullYear()} IT SPACE
                                </span>
                                <span class="fs-18p p-5p bsbb center mt--10p">
                                .
                                </span>
                                <a href="${geturl()}/" class="td-none ls-n"><span class="nowrap hover-6 dgray">Home</span></a>
                                <span class="fs-18p p-5p bsbb center mt--10p">
                                .
                                </span>
                                <a href="${geturl()}/about/" class="td-none ls-n"><span class="nowrap hover-6 dgray">About us</span></a>
                                <span class="fs-18p p-5p bsbb center mt--10p">
                                .
                                </span>
                                <a href="${geturl()}/terms/" class="td-none ls-n"><span class="nowrap hover-6 dgray">Terms and conditions</span></a>
                            </span>
                            <span class="center-2 capitalize verdana fs-10p p-10p bsbb">
                                website designed & developed by <a href="https://blogsite-woad.vercel.app/" class="td-none consolas theme" target="_blank">#himself__</a> 
                            </span>
                        </span>
                    </p>
                </div>
            </div>
        </div>
        </div>`