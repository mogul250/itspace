...
async function getProdInfo(product){
    let p = await query(`SELECT products.id as prodid,COUNT(feedbacks.id) as fb_count, COALESCE(AVG(feedbacks.rate),0) as fb_rate, products.quantity,products.availability,products.description, products.name as pname, products.specifications as pspecs,JSON_EXTRACT(products.conditions, '$') AS conditions,products.images as pimgs, products.orders as porders, categories.name as catname, subcategories.name as subcatname, brands.name as brandname,families.name as famname, usedin.name as usedinname FROM ((((((products inner join brands on products.brand = brands.name)inner join families on products.family = families.name)inner join categories on products.category = categories.name)inner join subcategories on  products.subcategory = subcategories.name)inner join usedin on products.usedin = usedin.name)left join feedbacks on products.id = feedbacks.product)  where products.id = '${product}' group by prodid`);
    if (p && p.length) {
        try {
